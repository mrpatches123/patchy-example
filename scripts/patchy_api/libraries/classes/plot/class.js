...
export class PlotBuilder {
            ...
            eventBuilder.subscribe('init_zplotCreateQueue', {
                worldLoad: () => {
                    ...
                    const runId = system.runInterval(() => {
                        ...
                        if (!keys.length)
                            return (console.warn('ehy'), plotThis.subscribedQueue = false, system.clearRun(runId), global.finishedInitialPlotCreate = true);
                        ...
                        keys.forEach(key => {
                            ...
                            let maxZ = 0, maxX = 0;
                            const reduceCallback = ((sumation, ruleSet, i) => {
                            ...
                            const generatedRuleSets = ruleSets.reduce(reduceCallback, []);
                            plotThis.plots[key] = {};
                            ...
                            if (hasBeenSubscribed || rules?.exclusive)
                                plotThis.subscribe();
                            if (!rules?.exclusive) {
    ...
    create(key, rules) {
        ...
        if (size && !(size instanceof BlockAreaSize))
        ...
        const indexsSize = ruleSets.reduce((sumation, current, i) => { if (!(current.size instanceof BlockAreaSize)) {
            sumation.push(i);
            return sumation;
        }
        else
    ...
    add(player, key, plotNumber) {
        ...
        if (!plot)
            throw new Error(`plot: ${key}, does not exist`);
        ...
        if (teleport) {
            ...
            wait.add(`${player}*plot*structure`, () => {
                ...
                if (key) {
                    ...
                    if (isVector3(face))
                        face = { location: startLocation, offset: face };
                    // content.warn(native.stringify(face));
                    teleportBuilder.teleportOnce(player, { location: teleportLocation, face: face, dimension: overworld });
                    return true;
                }
