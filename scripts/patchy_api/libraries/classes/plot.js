...
export class PlotBuilder {
    ...
    create(key, rules) {
        ...
        if (!(rules instanceof Object))
            throw new Error(`rules in rules at params[1] is not of type: Object!`);
        let { size, exclusive, start, ruleSets = [], property, plotNumberIdentifier, structure, teleport, allowMultiplePlayers, loop = false, loopDirection, defaultPermision, defaultGamemode } = rules;
        // content.warn({ plotNumberIdentifier });
        if (!exclusive && typeof plotNumberIdentifier !== 'string')
            throw new Error('plotNumberIdentifier in rules at params[1] is not of type: String!');
        if (!exclusive && typeof property !== 'boolean')
            throw new Error('plotNumberIdentifier in rules at params[1] is not of type: Boolean!');
        if (size && !isVector3(size))
        ...
        if (!start)
            throw new Error(`start, in rules at params[1] is not defined!`);
        if (!isVector3(start))
            throw new Error(`start, in rules at params[1] is not of type: Vector3!`);
        if (typeof loop !== 'boolean')
            throw new Error(`loop, in rules at params[1] is defined and not of type: Boolean!`);
        if (!loop && loopDirection)
            throw new Error(`loopDirection, in rules at params[1] is defined and loop is not defined!`);
        if (loopDirection && !directions.includes(loopDirection))
            throw new Error(`loopDirection, in rules at params[1] is defined and not one of the following: ${orArray(directions)}`);
        if (defaultPermision && !permisions.includes(defaultPermision))
            throw new Error(`defaultPermision, in rules at params[1] is defined and is not one of the following: ${orArray(permisions)}!`);
        if (defaultGamemode && !gamemodes.includes(defaultGamemode))
            throw new Error(`defaultGamemode, in rules at params[1] is defined and is not one of the following: ${orArray(gamemodes)}!`);
        if (isDefined(allowMultiplePlayers) && typeof allowMultiplePlayers !== 'boolean')
            throw new Error(`allowMultiplePlayers, in rules at params[1] is defined and not of type: Boolean!`);
        if (structure) {
            if (!(structure instanceof Object))
                throw new Error(`structure, in rules at params[1] is defined and not of type: Object!`);
            ...
            if (typeof name !== 'string')
                throw new Error(`name in structure in rules at params[1] is not of type: String!`);
            if (!isVector3(location))
                throw new Error(`location in structure in rules at params[1] is not of type: Vector3!`);
            if (!rotations.includes(rotation))
                throw new Error(`rotation in structure in rules at params[1] is not one of the following: ${orArray(rotations)}!`);
            if (!mirrors.includes(mirror))
                throw new Error(`mirror in structure in rules at params[1] is not one of the following: ${orArray(mirrors)}!`);
            if (!animationModes.includes(animationMode))
                throw new Error(`animationMode in structure in rules at params[1] is not one of the following: ${orArray(animationModes)}!`);
            if (typeof animationSeconds !== 'number')
                throw new Error(`animationSeconds in structure in rules at params[1] is not of type: Number!`);
            if (typeof includesEntites !== 'boolean')
                throw new Error(`includesEntites in structure in rules at params[1] is not of type: Boolean!`);
            if (typeof includesBlocks !== 'boolean')
                throw new Error(`includesBlocks in structure in rules at params[1] is not of type: Boolean!`);
            if (typeof waterlogged !== 'boolean')
                throw new Error(`waterlogged in structure in rules at params[1] is not of type: Boolean!`);
            if (typeof integrity !== 'number')
                throw new Error(`integrity in structure in rules at params[1] is not of type: Number!`);
            if (seed && typeof seed !== 'string')
                throw new Error(`seed in structure in rules at params[1] is not of type: String!`);
        }
        if (exclusive === true) {
            if (isDefined(structure))
                throw new Error(`structure, in rules at params[1] should not be defined as exclusive in rules at params[1] is true!`);
            ...
            if (isDefined(loopDirection))
                throw new Error(`loopDirection, in rules at params[1] should not be defined as exclusive in rules at params[1] is true!`);
            if (ruleSets.length > 1)
                throw new Error(`ruleSet's, in rules at params[1], length should not be greater than 1 as exclusive in rules at params[1] is true!`);
            if (isDefined(plotNumberIdentifier))
                throw new Error(`plotNumberIdentifier, in rules at params[1] should not be defined as exclusive in rules at params[1] is true!`);
            if (isDefined(property))
                throw new Error(`property, in rules at params[1] should not be defined as exclusive in rules at params[1] is true!`);
        }
        if (teleport) {
            if (!(teleport instanceof Object))
                throw new Error(`structure, in rules at params[1] is defined and not of type: Object!`);
            const { location, face, key } = teleport;
            if (key && typeof key !== 'string')
                throw new Error('key, in rules at params[1] is defined and not of type: String!');
            if (!key) {
                if (!isVector3(location))
                    throw new Error(`location, in teleport in rules at params[1] is not of type: Vector3!`);
                if (face && !isVector2(face))
                    throw new Error(`face, in teleport in rules at params[1] is defined and not of type: Vector2 or Vector3!`);
                if (face && !isVector2(face))
                    throw new Error(`face, in teleport in rules at params[1] is defined and not of type: Vector2 or Vector3!`);
            }
        }
        ...
        const newRuleSets = (rules.ruleSets) ? [] : undefined;
        ...
        rules?.ruleSets?.forEach((ruleSet, i) => {
            ...
            if (isVector3(direction) && isVector3(count)) {
                const offsetVectorForDirection = Vector.multiply(direction, sizeRuleSet ?? size);
                ...
                for (let z = 0; z < newCount.z; z++) {
                    for (let y = 0; y < count.y; y++) {
                        for (let x = 0; x < count.x; x++) {
                            ...
                            clone.start = vectorToVector3(Vector.add(Vector.multiply(Vector.add(offset ?? Vector.zero, offsetVectorForDirection), plotRelitive), clone.start));
        ...
        if (!exclusive && !allowMultiplePlayers) {
            let plots = databases.get('plots*API') ?? databases.add('plots*API');
            ;
            if (!plots.has(key)) {
                plots.set(key, { availablePlots: [0], currentIndex: 0 });
    ...
    setCurrent(player, key) {
        ...
        if (!key)
            return;
        const ruleSet = this.plots[key].rules;
        let { exclusive, teleport = {}, start, size } = ruleSet;
        if (teleport) {
            ...
            if (teleportKey)
                ...
                if (!isVector2(face))
                    face = { location: start, offset: face };
    ...
    getRuleSet(key, number) {
        ...
        if (typeof number !== 'number')
            throw new Error('why is number not defined');
        if (!loop)
            return ruleSets[number];
            ...
            tickAfterLoad: () => {
                players.get().iterate((player) => {
                    ...
                    if (isDefined(gamemodeOveride)) {
                        ...
                        if (gamemodePlayer !== gamemodeOveride)
                            player.gamemode = gamemodeOveride;
                    }
                    ...
                    if (betweenBlockVector3(location, start, end))
                        return;
                    if (betweenBlockVector3(lastLocation, start, end)) {
                        ...
                        if (teleport) {
                            ...
                            if (key)
                                teleportBuilder.teleport(player, key);
                            else {
                                teleportLocation = { location: start, offset: teleportLocation };
                                if (!isVector2(face))
                                    face = { location: start, offset: face };
            ...
            beforeItemUseOn: (event) => {
                ...
                switch (permision) {
                    ...
                    case 'write': {
                        ...
                        if (betweenBlockVector3(blockPlaceLocation, start, end))
                            return;
                    }
                    case 'break':
                    case 'read': {
    ...
    add(player, key, plotNumber) {
        ...
        const { plotNumberIdentifier, property, loop, teleport, structure, ruleSets = [], allowMultiplePlayers } = this.plots[key].rules ?? {};
        const queryNumber = this.query(player, key);
        if (isDefined(queryNumber))
            return { wasAdded: false, plotNumber: queryNumber, full: false };
        const plots = databases.get('plots*API');
        if (!plots)
            throw new Error('why does the plots*API db not exist');
        const plot = plots.get(key);
        let { availablePlots, currentIndex, hasBeenSubscribed } = plot ?? {};
        if (!loop && ruleSets.length === currentIndex && !availablePlots.length)
            return { wasAdded: false, full: true };
        if (allowMultiplePlayers) {
            if (!isDefined(plotNumber))
                throw new Error(`plotNumber at params[2] is not defined while allowMultiplePlayers for key: ${key}, is true`);
        }
        else if (!isDefined(plotNumber)) {
            plotNumber = availablePlots.shift();
            if (availablePlots.length === 0)
                availablePlots.push(++currentIndex);
        }
        else if (availablePlots.includes(plotNumber))
            availablePlots = availablePlots.filter(number => number !== plotNumber);
        ...
        wait.add(`${player}*plot*structure`, () => {
            const test = (!isDefined(loadId)) ? true : structureBuilder.getLoadStatus(loadId).done;
            content.warn({ test, loadId, done: structureBuilder.getLoadStatus(loadId)?.done });
            return test;
        }, () => {
            this.setCurrent(player, key);
            return true;
