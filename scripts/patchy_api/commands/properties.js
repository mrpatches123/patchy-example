import { commandBuilder, formBuilder, isVector3, players, propertyManager } from '../modules.js';
...
function parsePropertyValue(string) {
    ...
    try {
        ...
        if (isVector3(vector3))
            return vector3;
    }
    catch { }
    return string;
}
function parsePlayer(sender, playerName) {
    let player;
    if (playerName === 's') {
        player = sender;
    }
        ...
        if (!player) {
            sender.sendMessage(`player: ${playerName}, at params[2] does not exist!`);
            return false;
        }
    }
    return player;
